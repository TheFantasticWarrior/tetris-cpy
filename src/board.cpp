...
/*
   int8_t game::check_tetris_hole(int8_t y,int8_t *filled,int8_t*
bottom_filled,int8_t *empty)
   {
   int8_t* filled = 0;
   int8_t nempty=0;
   int8_t col_filled;
...
if (x==2)
{
    ...
    completeness = corners_filled / 3. * (board[y - 4][x - 2] == -1 ? 1 :
board[y - 5][x - 2] != -1 ? 1 : 0.5);
}
if (completeness)
{
    return completeness;
...
if (x <= 7) {
    ...
    if (x == 2)
    {
        ...
        completeness = (corners_filled > 2 ? 1 : corners_filled / 3.) * (board[y
- 4][x - 2] == -1 ? 1 : board[y - 5][x - 2] != -1 ? 1 : 0.5);
    }
    return completeness;
...
p2:
    if (x <= 7) {
        ...
        if (x == 2)
        {
            ...
            completeness = (corners_filled > 2 ? 1 : corners_filled / 3.) *
(board[y - 4][x - 2] == -1 ? 1 : board[y - 5][x - 2] != -1 ? 1 : 0.5);
        }
        return completeness;
    ...
    while(i>10){
        ...
        else if (nempty > 0)
        {
            for (size_t j = 0; j < nempty; j++)
            {
                ...
                if ((x=check_tst_hole(i, hole[j], &filled)))score += filled * x
/ 260.;
            }
    ...
    for (int8_t i : {0, 1}) {
      for (int8_t j : {0, 1}) {
        if (y + 1 + i > 29 || x + 1 + j < 0 || x + 1 + j > 10) {
          ...
          if ((i << 1) + j == front[rotation] ||
              (i << 1) + j == front[mod(rotation + 1, 4)]) {
            frontcount++;
          }
          block++;
        }
  ...
  while (!list.empty()) {
    ...
    if (incoming > 0) {
      ...
      for (size_t j = 0; j < incoming; j++) {
        for (size_t i = 0; i < 10; i++) {
          board[29 - j][i] =
              (i == x || (incoming >= 2 && thole && (j == incoming - 1) &&
                          (i == x - 1 || i == x + 1)))
                  ? -1
                  : 7;
        }
...
int8_t game::softdropdist() const {
  ...
  for (int8_t i = 0; i < 4; i++) {
    if (bottom[active][rotation][i] != 4) {
      int8_t j = 0;
      while (board[j + y + 3 - bottom[active][rotation][i]][x + i] == -1 &&
             j + y - bottom[active][rotation][i] < 27) {
        j++;
      }
      height[i] = j - 1;
    }
...
void game::sd() {
  ...
  for (int8_t i = 0; i < 4; i++) {
    if (bottom[active][rotation][i] != 4 &&
        !(board[1 + y + 3 - bottom[active][rotation][i]][x + i] == -1 &&
          1 + y - bottom[active][rotation][i] < 27)) {
      goto end;
    }
  }
  y += 1;
...
int8_t mod(int8_t x, int8_t y) { return ((x % y) + y) % y; }
...
void game::copy_board(int8_t dest[ROWS][COLUMNS],
                      const int8_t src[ROWS][COLUMNS]) {
